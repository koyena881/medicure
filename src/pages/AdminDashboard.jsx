...
export default function AdminDashboard({ role = 'main' }) {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState(role === 'main' ? 'manage' : 'dashboard'); // dashboard, manage
    ...
    <div className="app-container">
      <aside className="sidebar">
        <div style={{ padding: '0 32px 32px 32px', textAlign: 'center' }}>
          ...
          <h2 className="text-gradient-blue title-display" style={{ fontSize: '1.8rem', margin: 0 }}>
            {role === 'main' ? 'Main Admin' : 'Sub Admin'}
          </h2>
          <div className="flex-center" style={{ gap: '6px', fontSize: '0.8rem', color: 'var(--accent-green)', marginTop: '8px' }}>
            <ShieldCheck size={14} /> {role === 'main' ? 'Doctor Registry' : 'Appointment Desk'}
          </div>
        </div>
        <nav style={{ flex: 1 }}>
          {role === 'sub' && (
          ...
          <a className={`nav-item ${activeTab === 'manage' ? 'active' : ''}`} onClick={() => setActiveTab('manage')}>
            {role === 'main' ? <Users size={20} /> : <Calendar size={20} />}
            {role === 'main' ? ' Manage Doctors' : ' Appointments'}
          </a>
      ...
      <main className="main-content">
        <header className="dashboard-header">
          <div>
            <h1>{activeTab === 'dashboard' ? 'Command Centre' : (role === 'main' ? 'Medical Staff Control' : 'Appointment Registry')}</h1>
            <p>{activeTab === 'dashboard' ? 'Observing real-time clinical activity.' : (role === 'main' ? 'Verify credentials and manage active doctors.' : 'Finalize and approve patient bookings.')}</p>
          </div>
        ...
        {activeTab === 'dashboard' ? (
          ...
          <div className="dashboard-grid" style={{ gridTemplateColumns: role === 'main' || role === 'sub' ? '1fr' : '1fr 1fr' }}>
            {/* Doctor Credential Verification Segment (Main Admin Only) */}
            {role === 'main' && (
              <section className="glass-panel">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '16px' }}>
                  ...
                  {doctors.filter(d => d.status === 'pending').length === 0 && (
                    <div style={{ textAlign: 'center', padding: '40px', opacity: 0.5, gridColumn: '1 / -1' }}>
            ...
            {/* Appointment Management Segment (Sub Admin Only) */}
            {role === 'sub' && (
              <section className="glass-panel">
